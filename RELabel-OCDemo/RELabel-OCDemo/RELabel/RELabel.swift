...
class RELabel: UILabel {
    ...
    /// 设置要匹配的结果类型
    var matchTypes : [TapHandlerType] = [.UserTapHandler, .LinkTapHandler, .TopicTapHandler]{
        didSet{
            prepareText()
        }
    }
    /// 匹配自定义的文字（默认用户、连接、话题 不再匹配）
    var customMatchString : String?{
        didSet{
            
            guard customMatchString != nil else { return }
            matchTypes = [.NoneTapHandler]
            prepareText()
        }
    }
    ...
    fileprivate lazy var costomRanges : [NSRange] = [NSRange]()
    ...
    var customTapHandler : RETapHandler?
...
extension RELabel {
    ...
    fileprivate func prepareText() {
        ...
        for type in matchTypes {
            ...
            case .NoneTapHandler:
                matchCustomString(self.customMatchString ?? "")
                break
            case .UserTapHandler:
                matchUser()
                break
    ...
    fileprivate func matchCustomString(_ customStr : String){
        // 7.匹配自定义的string
        if let customRanges = getRanges(customStr) {
            self.costomRanges = customRanges
...
extension RELabel {
    ...
    override open func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // 3.回调(判断不是自定义match)
        guard !matchTypes.contains(.NoneTapHandler) else {
            if customTapHandler != nil {
                customTapHandler!(self, contentText, selectedRange!)
            }
            return  }
    ...
    fileprivate func getSelectRange(_ selectedPoint : CGPoint) -> NSRange? {
        ...
        // -1 如果是自定义match就不用匹配后面了
        if !matchTypes.contains(.NoneTapHandler) {
            ...
            // 2.4.判断是否是一个coustomString
            for customRange in costomRanges{
                if index > customRange.location && index < customRange.location + customRange.length {
                    setNeedsDisplay()
                    tapHandlerType = .NoneTapHandler
                    return customRange
                }
            }
        }
